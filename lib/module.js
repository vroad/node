...
Module.prototype._compile = function(content, filename, outCodeCache, produceCodeCache) {
  var self = this;
  // remove shebang
  ...
  var options = { filename: filename, codeCache: outCodeCache != null ? outCodeCache.data : null, produceCodeCache: produceCodeCache };
  var compiledWrapper = runInThisContext(wrapper, options);
  if (outCodeCache != null)
    outCodeCache.data = options.codeCache;
  if (global.v8debug) {
    if (!resolvedArgv) {
...
Module._extensions['.js'] = function(module, filename) {
  ...
  var cacheObject = { data: null };
  var cacheFileName = filename + '.codecache';
  try {
    fd = fs.openSync(cacheFileName, 'r');
    var cacheStats = fs.fstatSync(fd);
    if (cacheStats.mtime >= stats.mtime) {
      cacheObject.data = new Buffer(cacheStats.size);
      fs.readSync(fd, cacheObject.data, 0, cacheStats.size);
    }
  } catch(e) {}
  finally
  {
  ...
  module._compile(internalModule.stripBOM(content), filename, cacheObject, true);
  if (cacheObject.data != null)
    fs.writeFileSync(cacheFileName, cacheObject.data)
};
